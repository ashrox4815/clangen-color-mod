...
class Pelt():
    ...
    pelt_colours = [
        ...
        'CHOCOLATE', 'PINK', 'RED', 'LIME', 'DARKGREEN', 'CYAN', 'DARKBLUE', 'PURPLE'
    ]
    pelt_c_no_white = [
    ...
    tortiepatterns = ['ONE', 'TWO', 'THREE', 'FOUR', 'REDTAIL', 'DELILAH', 'MINIMALONE', 'MINIMALTWO', 'MINIMALTHREE', 'MINIMALFOUR', 'HALF',
                    'OREO', 'SWOOP', 'MOTTLED', 'SIDEMASK', 'EYEDOT', 'BANDANA', 'PACMAN', 'STREAMSTRIKE', 'ORIOLE', 'CHIMERA', 'DAUB', 'BLANKET',
    ...
    pink_colours = ['PINK', 'RED', 'PURPLE']
    blue_colours = ['LIME', 'DARKGREEN', 'CYAN', 'DARKBLUE']
    colour_categories = [ginger_colours, black_colours, white_colours, brown_colours, pink_colours, blue_colours]
    eye_sprites = [
    ...
    def pattern_color_inheritance(self, parents: tuple=(), gender="female"):
        ...
        #   PELT COLOUR
        # ------------------------------------------------------------------------------------------------------------#
        # Weights for each colour group. It goes: (ginger_colours, black_colours, white_colours, brown_colours, pink_colours, blue_colours)
        weights = [0, 0, 0, 0, 0, 0]
        for p_ in par_peltcolours:
            if p_ in Pelt.ginger_colours:
                add_weight = (40, 0, 0, 10, 20, 0)
            elif p_ in Pelt.black_colours:
                add_weight = (0, 40, 2, 5, 0, 10)
            elif p_ in Pelt.white_colours:
                add_weight = (0, 5, 40, 0, 0, 5)
            elif p_ in Pelt.brown_colours:
                add_weight = (10, 5, 0, 35, 5, 5)
            elif p_ in Pelt.pink_colours:
                add_weight = (15, 0, 0, 10, 40, 5)
            elif p_ in Pelt.blue_colours:
                add_weight = (0, 10, 2, 2, 5, 40)
            elif p_ is None:
            ...
            if self.colour:
                ...
                if not wildcard_chance or random.getrandbits(wildcard_chance) == 1:
                    ...
                    if (self.colour in Pelt.black_colours) or (self.colour in Pelt.white_colours):
                        self.tortiecolour = choice((Pelt.ginger_colours * 2) + Pelt.brown_colours + (Pelt.pink_colours * 2) + (Pelt.blue_colours * 2))
                    elif self.colour in Pelt.ginger_colours:
                        self.tortiecolour = choice(Pelt.brown_colours + Pelt.black_colours * 2 + Pelt.blue_colours * 2)
                    elif self.colour in Pelt.brown_colours:
                        possible_colors = Pelt.brown_colours.copy()
                        possible_colors.remove(self.colour)
                        possible_colors.extend(Pelt.black_colours + (Pelt.ginger_colours * 2) + Pelt.blue_colours + Pelt.pink_colours)
                        self.tortiecolour = choice(possible_colors)
                    elif self.colour in Pelt.blue_colours:
                        possible_colors = Pelt.blue_colours.copy()
                        possible_colors.remove(self.colour)
                        possible_colors.extend((Pelt.black_colours * 2) + (Pelt.ginger_colours * 2) + (Pelt.pink_colours * 2) + Pelt.brown_colours)
                        self.tortiecolour = choice(possible_colors)
                    elif self.colour in Pelt.pink_colours:
                        self.tortiecolour = choice(Pelt.brown_colours + Pelt.black_colours * 2 + Pelt.blue_colours * 2)
                    else:
